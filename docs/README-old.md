...
**rax25kb** is a bridge application that connects a serial port running
...
:   Specify the serial port device. On Linux, this is typically
    */dev/ttyUSB0* or */dev/ttyACM0*. On Windows, use *COM3* or similar.
...
:   Set parity checking. KISS TNC default is none. Values: **n** (none),
    **e** (even), **o** (odd).
...
:   Specify one or more TCP listener addresses, separated by spaces or
    ...
    (localhost only), **::** (all IPv6 interfaces), **::1** (IPv6
...
:   Enable PhilFlag correction. This flag applies corrections for
    ...
    -   **Serial -\> TCP:** Replace unescaped FEND bytes (0xC0) inside
...
:   Serial port flow control. Values: **none**, **software**,
    **hardware**, **dtrdsr**. Alternative spellings accepted: **xon**,
...
:   Escapes \'C\' (0x43) and \'c\' (0x63) characters as the sequences
    (0xDB 0x43) and (0xDB 0x63) to prevent the radio from
    misinterpreting \"TC0\\n\" or \"tc0\\n\" as command sequences. This
    prevents unintended mode changes or radio resets.
...
port device. This typically requires membership in the **dialout** or
**uucp** group:
